...
def addToCart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__pk=item.pk).exists():
            order_item.qty += 1
            order_item.save()
            messages.info(request, 'Quantity of ' +
    ...
    if cart_qs.exists():
        ...
        if cart.items.filter(item__pk=item_to_remove.pk).exists():
            ...
            if order_item.qty > 1:
                order_item.qty -= 1
                order_item.save()
                messages.info(request, 'Removed 1 ' +
                              item_to_remove.name + ' from your cart')
            else:
            ...
            messages.info(request, item_to_remove.name +
                          ' is not present in your cart')
            return redirect('showCake', pk=pk)
...
@login_required(login_url='/')
def removeFromCart_OS(request, pk):
    item_to_remove = get_object_or_404(Product, pk=pk)
    cart_qs = Cart.objects.filter(
        user=request.user,
        ordered=False
